...
function JobCard({ job }: { job: Job }) {
  ...
  return (
    <Link href={`/jobs/${job._id}`} className="block">
      <div className="group relative bg-white border border-surface-border rounded-2xl p-5
        ...
        <button
          ...
          className="absolute top-4 right-4 text-ink-faint hover:text-lime transition-colors"
        ...
        <div className="flex items-start gap-3 pr-6">
          <div className="w-10 h-10 rounded-xl bg-surface-tertiary border border-surface-border
                          flex-shrink-0 flex items-center justify-center text-lime font-bold
                          text-sm font-poppins group-hover:bg-lime-light transition-colors">
          ...
          <div className="min-w-0">
            ...
            <p className="text-ink-muted text-xs mt-0.5">{job.company}</p>
          </div>
        </div>
        ...
        {job.tags && job.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-3 mt-auto border-t border-surface-border">
            {job.tags.slice(0, 4).map(tag => (
              <span key={tag}
                className="text-xs text-ink-muted bg-surface-tertiary px-2 py-0.5 rounded-md
                           hover:bg-lime-light hover:text-lime transition-colors">
                {tag}
              </span>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-7">
          ...
          <aside className="w-full lg:w-64 flex-shrink-0">
            <div className="bg-white border border-surface-border rounded-2xl p-5 sticky top-24 space-y-6 shadow-card">
              <div className="flex items-center justify-between">
                <p className="font-poppins font-semibold text-ink">Filters</p>
              ...
              <div className="relative">
                <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-ink-faint"
                ...
                <input type="text" value={q} onChange={e => setQ(e.target.value)}
                  ...
                  className="w-full bg-surface-tertiary border border-surface-border rounded-lg
                             pl-9 pr-3 py-2 text-ink placeholder-ink-faint text-sm
                             focus:outline-none focus:border-lime/50" />
              </div>

              {/* Location */}
              <div>
                <p className="text-xs font-semibold text-ink-muted uppercase tracking-wider mb-2">Location</p>
                {[{ label: 'All Locations', value: '' }, ...visibleLocs.map(l => ({ label: l, value: l }))].map(({ label, value }) => (
                  <label key={label} className="flex items-center gap-2 py-1.5 cursor-pointer group">
                    ...
                    <span className="text-sm text-ink-muted group-hover:text-ink transition-colors flex-1">{label}</span>
              ...
              {/* Batch year */}
              <div>
                <p className="text-xs font-semibold text-ink-muted uppercase tracking-wider mb-2">Batch Year</p>
              ...
              {/* Job type */}
              <div>
                <p className="text-xs font-semibold text-ink-muted uppercase tracking-wider mb-2">Job Type</p>
          ...
          <div className="flex-1 min-w-0">
            <div className="mb-6">
              <h1 className="font-poppins font-bold text-2xl text-ink">Explore Jobs</h1>
            ...
            <div className="flex items-center justify-between mb-5">
              <p className="text-ink-muted text-sm">
                <span className="text-ink font-semibold">{jobs.length}</span> jobs found
              </p>
              <select value={sort} onChange={e => setSort(e.target.value)}
                className="bg-white border border-surface-border rounded-lg px-3 py-1.5
                           text-ink text-sm focus:outline-none focus:border-lime/50 cursor-pointer">
            ...
            <AnimatePresence mode="wait">
              {jobs.length > 0 ? (
                ...
                <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  className="bg-white border border-surface-border rounded-2xl p-12 text-center shadow-card">
                  <p className="font-poppins font-semibold text-ink text-lg mb-2">No jobs found</p>
