...
export const metadata: Metadata = {
  title: 'Resume Hub — Builders, ATS Checkers, Writing Guides & LinkedIn Tips',
  description:
    'Everything you need to build a resume that gets noticed — builders, ATS checkers, writing tips, and LinkedIn optimization tools.',
};
    ...
    <div className="pt-20 min-h-screen pb-20" style={{ background: '#0A0A0A' }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
        ...
        <nav className="text-sm text-ink-muted mb-8 flex items-center gap-2 flex-wrap">
          ...
          <span className="text-ink-faint">/</span>
          <span className="text-ink">Resume Hub</span>
        </nav>
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-ink-muted text-base">
            Everything you need to build a resume that gets noticed.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {categories.map(cat => (
            <Link
              key={cat.id}
              href={`/resources/resume-hub/${cat.id}`}
            ...
            >
              ...
              >
                {cat.tools.length} {cat.tools.length === 1 ? 'resource' : 'resources'}
              </span>
            </Link>
          ))}
        </div>
