...
const roadmapData: Record<string, RoadmapData> = {
  frontend: {
    ...
    icon: '🌐', level: 'Beginner', duration: '3–4 months', totalTime: '24h 30m',
  ...
  dsa: {
    ...
    icon: '🧩', level: 'Intermediate', duration: '3 months', totalTime: '30h',
  ...
  backend: {
    ...
    icon: '⚙️', level: 'Intermediate', duration: '3–4 months', totalTime: '28h',
  ...
  'system-design': {
    ...
    icon: '🏗️', level: 'Advanced', duration: '2 months', totalTime: '20h',
  ...
  aiml: {
    ...
    icon: '🤖', level: 'Beginner', duration: '4 months', totalTime: '22h',
  ...
  devops: {
    ...
    icon: '🔧', level: 'Advanced', duration: '4 months', totalTime: '35h',
    ...
    nodes: [
      ...
      { title: 'Kubernetes',        status: 'locked',       resources: [{ label: 'K8s Crash Course',                 url: 'https://youtube.com', type: 'video' }] },
...
const allRoadmapsForNav = Object.entries(roadmapData).map(([id, d]) => ({ id, title: d.title, icon: d.icon }));
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="glass-card p-6 md:p-8 mb-8">
              ...
              <div className="flex flex-wrap gap-6 text-sm mb-5">
                <span className="text-gray-400">📚 <span className="text-white font-medium">{data.modules} Modules</span></span>
                <span className="text-gray-400">⏱ <span className="text-white font-medium">{data.totalTime} Total Time</span></span>
                <span className="text-gray-400">🎯 <span className="text-lime font-medium">{pct}% Completed</span></span>
                <span className="text-gray-400">📊 <span className="text-white font-medium">{data.level}</span></span>
              </div>
              ...
              {data.completedModules > 0 && (
                <div>
                  ...
                  <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-lime to-lime-bright rounded-full" style={{ width: `${pct}%` }} />
                  </div>
                </div>
              )}
            </div>
            ...
            {(() => {
              ...
              if (!next) return null;
              return (
                <div className="glass-card p-5 mb-8 border border-lime/20 bg-lime/5">
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div>
                      <p className="text-xs text-lime font-semibold uppercase tracking-wider mb-1">Next Up</p>
                      <h3 className="font-poppins font-bold text-white">{next.title}</h3>
                      <p className="text-gray-400 text-sm mt-1">
                        {next.resources[0]?.label}
                      </p>
                    </div>
                    <a href={next.resources[0]?.url || '#'} target="_blank" rel="noopener noreferrer"
                       className="btn-lime text-sm py-2.5 px-6 whitespace-nowrap">
                      Continue Learning →
                    </a>
                  </div>
                </div>
